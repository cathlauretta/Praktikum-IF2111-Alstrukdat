...
*/
{
	...
	{
		Insert(&UMap, m1.Elements[i].Key, m1.Elements[i].Value);
	}

	for (j = 0; j < m2.Count; j++)
	{
		Insert(&UMap, m2.Elements[j].Key, m2.Elements[j].Value);
	}

	if (key_based == 1)
	{
		for (i = 0; i < UMap.Count; i++)
		{
			for (j = i; j < UMap.Count; j++)
			{
				if (UMap.Elements[j].Key < UMap.Elements[i].Key)
				{
					infotype temp = UMap.Elements[i];
					UMap.Elements[i] = UMap.Elements[j];
					UMap.Elements[j] = temp;
				}
			}
		}
	}
	else
	{
		for (i = 0; i < UMap.Count; i++)
		{
			for (j = i; j < UMap.Count; j++)
			{
				if (UMap.Elements[j].Value < UMap.Elements[i].Value)
				{
					infotype temp = UMap.Elements[i];
					UMap.Elements[i] = UMap.Elements[j];
					UMap.Elements[j] = temp;
				} else if (UMap.Elements[j].Value == UMap.Elements[i].Value)
				{
					if (UMap.Elements[j].Key < UMap.Elements[i].Key)
					{
						infotype temp = UMap.Elements[i];
